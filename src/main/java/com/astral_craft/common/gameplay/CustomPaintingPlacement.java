...
public class CustomPaintingPlacement {
    ...
    public static Vec3 bottomLeft(BlockPos supportPos, Direction facing) {
        Direction right = right(facing);
        return Vec3.atCenterOf(supportPos)
                .add(right.getStepX() * -0.5D, -0.5D, right.getStepZ() * -0.5D)
    ...
    public static Vec3 center(BlockPos supportPos, Direction facing, CustomPaintingData data) {
        Direction right = right(facing);
        return bottomLeft(supportPos, facing).add(right.getStepX() * data.width() * 0.5D,
                data.height() * 0.5D, right.getStepZ() * data.width() * 0.5D);
    }
    ...
    public static boolean canPlace(Level level, BlockPos supportPos, Direction facing, CustomPaintingData data, Entity ignored) {
        ...
        for (int x = 0; x < data.width(); x++) {
            for (int y = 0; y < data.height(); y++) {
                BlockPos support = supportPos.relative(right, x).above(y);
