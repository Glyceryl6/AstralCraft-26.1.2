package com.astral_craft.common.network;

import com.astral_craft.AstralCraft;
import io.netty.buffer.ByteBuf;
import net.minecraft.network.codec.ByteBufCodecs;
import net.minecraft.network.codec.StreamCodec;
import net.minecraft.network.protocol.common.custom.CustomPacketPayload;

public record OpenCardBackSelectionPayload(String encodedOptions, String selectedId) implements CustomPacketPayload {

    public static final CustomPacketPayload.Type<OpenCardBackSelectionPayload> TYPE = new CustomPacketPayload.Type<>(AstralCraft.prefix("open_card_back_selection"));

    public static final StreamCodec<ByteBuf, OpenCardBackSelectionPayload> STREAM_CODEC = StreamCodec.composite(
            ByteBufCodecs.STRING_UTF8, OpenCardBackSelectionPayload::encodedOptions,
            ByteBufCodecs.STRING_UTF8, OpenCardBackSelectionPayload::selectedId,
            OpenCardBackSelectionPayload::new);

    @Override
    public Type<? extends CustomPacketPayload> type() {
        return TYPE;
    }

}