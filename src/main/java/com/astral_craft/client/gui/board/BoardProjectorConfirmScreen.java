...
public class BoardProjectorConfirmScreen {
    ...
    public static void open(OpenBoardProjectorConfirmPayload payload, IPayloadContext context) {
        context.enqueueWork(() -> Minecraft.getInstance().setScreen(new AstralConfirmationScreen(
                Component.translatable("gui.astral_craft.board_projector.confirm.title"),
                List.of(Component.translatable("gui.astral_craft.board_projector.confirm.warning"),
                ...
                () -> ClientPacketDistributor.sendToServer(new BoardProjectorConfirmPayload(
                        payload.groundPos(), payload.facing(), payload.offhand())))));
    }
}
